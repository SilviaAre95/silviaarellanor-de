...
acceptance_criteria:
  ...
  - Every resume compiles into a two-page, text-selectable, ATS-readable PDF using the approved editorial-technical layout and teal branding.
...
non_goals:
  ...
  - Reproduce the old Canva PDF pixel-for-pixel or use photos, sidebars, skill charts, and decorative graphics that reduce ATS readability.
...
The system does not compile documents in the browser, publish non-Senior variants, fabricate claims, tailor itself to arbitrary job descriptions, reproduce the old Canva layout exactly, submit applications, or commit one-off application artifacts by default. Job-description tailoring belongs to a separate repository skill.
