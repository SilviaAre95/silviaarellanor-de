...
The categories and skill cards are defined in `src/pages/Skills/Skills.jsx`. The section sits on the same foam ground as the hero, with the brand wave marking the top-right corner (masked at its left edge so the horizon carries out rather than cutting off mid-page). Each category is a rectangle taking one of the two accents in alternation, shown as a short rule above the heading and as the card's hairline edge; chips are outlined rather than filled. Filling forty chips with chrome and sea made the palette the design; foam and abyss carry the page. Styling lives in the brand layer of `src/assets/css/index.css` (`.stack`, `.stackcard`, `.chip`) rather than in the shadcn card and badge components. Skill icons come from `react-icons` and `lucide-react` and inherit the chip's colour — vendor brand colours are outside the closed palette.
